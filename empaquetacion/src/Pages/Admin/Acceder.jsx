...
const AccederAdmin = () => {
    const { loading, setEmail, setPassword, LogInAdmin } = useAuth();

    return(
        <main className="flex p-8 Acceder h-full items-center justify-center min-h-screen">
            <form onSubmit={LogInAdmin} className="flex flex-col pt-15 pb-15 pl-40 pr-40 rounded-3xl Login items-center justify-between gap-6 w-3/5">
                ...
                <div className="flex flex-col gap-2 items-center justify-center">
                    ...
                    <Link to="/admin/Recuperacion" className="text-xs text-white">Recuperar contraseña</Link>
                </div>
            </form>
        </main>
    )
