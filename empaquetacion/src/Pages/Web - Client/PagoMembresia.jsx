...
const PagoMembresia = () => {
    ...
    const membresiaPorDefecto = {
        id: "MBR001",
        name: "Membresía",
        gimnasioNombre: "Active Life",
        precioUni: 0,
        PeriodoPago: "Mensualmente"
    };

    const membresiaSeleccionada = location.state?.membresia || membresiaPorDefecto;
    ...
    useEffect(() => {
        const monto = membresiaSeleccionada.precioUni || 0;
        setSubtotal(monto);
        setTotal(monto);
    }, []);

    const finalizarPago = async () => {
        if (!titular || !numeroTarjeta || !fechaExp || !cvv) {
            Swal.fire({ title: "Completa todos los campos", icon: "warning", timer: 2000, showConfirmButton: false });
            return;
        }
        ...
        try {
            const res = await fetch("http://localhost:4000/apiActiveLife/sales", {
                ...
                body: JSON.stringify({
                    ...
                    clientId: Id,
                    membresiaData: {
                        nombre: membresiaSeleccionada.name,
                        gimnasio: membresiaSeleccionada.gimnasioNombre,
                        precio: membresiaSeleccionada.precioUni,
                        periodo: membresiaSeleccionada.PeriodoPago
                    }
                }),
                credentials: "include",
            });
            ...
            if (res.ok) {
                await Swal.fire({ title: "¡Membresía activada!", text: "Tu membresía ha sido registrada exitosamente.", icon: "success", timer: 2500, showConfirmButton: false });
                navigate("/client/Gimnasios");
            } else {
            ...
            <div className="flex flex-row gap-4 p-6 rounded-3xl min-h-full" style={{display: 'grid', gridTemplateColumns: '4fr 2fr', background: 'var(--green_7F9E7A)'}}>
                            ...
                            <div className="flex flex-row gap-10 justify-start pl-3">
                                ...
                                <div className="text-white flex flex-col items-center gap-0 cursor-pointer">
                                    <svg className="p-0 m-0" xmlns="http://www.w3.org/2000/svg" width="2.5rem" height="2.5rem" fill="white" viewBox="0 0 2387.3 948">
                                        ...
                                        <path d="M172.9,469.7c-16.5-48.9-16.5-102,0-150.9V217.2H42c-56,111.4-56,242.7,0,354.1L172.9,469.7z" fill="#FBBC05"/>
                ...
                <div className="rounded-xl pl-6 pr-6 pt-3 pb-3 flex flex-col justify-between" style={{background: 'var(--green_CFD9C7)'}}>
                        ...
                        <div className="p-0 flex flex-col rounded-xl gap-3">
                            ...
                            >
                                <div className="flex flex-col gap-1">
                                    <p className="text-white font-regular text-sm leading-tight">{membresiaSeleccionada.name}</p>
                                    <p className="text-white/75 text-xs">{membresiaSeleccionada.gimnasioNombre}</p>
                                </div>
                                <div className="flex flex-col gap-1 items-end">
                                    <p className="text-white font-regular text-lg">${membresiaSeleccionada.precioUni?.toFixed(2)}</p>
                                    <span
                                        className="text-xs font-regular px-2 py-1 rounded-full w-fit"
                                        style={{ background: "var(--green_81A65D)", color: "#fff" }}
                                    >
                                        {membresiaSeleccionada.PeriodoPago}
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <button
                        ...
                        disabled={procesando}
                        className="flex items-center gap-4 justify-center text-white p-2 rounded-lg cursor-pointer transition hover:opacity-90 disabled:opacity-60"
