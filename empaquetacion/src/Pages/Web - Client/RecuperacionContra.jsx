...
const RecuperacionContra = () => {
    const { email, setEmail, codigo, setCodigo, setPassword, setConfirmPassword, loading, tab, EmailSolicitudeClient, VerificarCodigoClient, ChangePasswordClient } = useRecovery();
    
    return(
        <main className="flex p-8 Acceder h-full items-center justify-center min-h-screen">
            { tab === "solicitarCorreo" ? (
                <SolicitarCorreo setEmail={setEmail} loading={loading} EmailSolicitude={EmailSolicitudeClient} />
            ): tab === "verificarCodigo" ? (
                <VerificarCorreo codigo={codigo} setCodigo={setCodigo} loading={loading} verificarCorreo={VerificarCodigoClient} />
            ):  tab === "cambiarContra" ? (
                <CambiarContra setPassword={setPassword} setConfirmPassword={setConfirmPassword} loading={loading} ChangePassword={ChangePasswordClient} />
            ): null}
        </main>
    )
}
